...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Convierte a número de forma segura (quita separadores de miles)
    ...
    function drawSpark(el) {
        const trend = num(el.dataset.trend);
        const up = trend >= 0;
    ...
    table.querySelectorAll('th[data-sort-index]').forEach((th) => {
        th.addEventListener('click', () => {
            ...
            rows.sort((a, b) => {
                const av = num(a.children[idx] && a.children[idx].dataset.value);
                const bv = num(b.children[idx] && b.children[idx].dataset.value);
